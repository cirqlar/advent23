...
fn shorts_to_path_to_arrows(
    arrows: &[u8],
    times: usize,
    cache: &mut FxHashMap<Vec<u8>, Vec<u8>>,
) -> Vec<u8> {
    ...
    let mut prev = b'A';
    if cache.contains_key(&as_vec) {
        if times == 1 {
            return cache.get(&as_vec).unwrap().clone();
        } else {
            paths.push(cache.get(&as_vec).unwrap().clone());
        }
    } else {
    ...
    let fin = if times == 1 {
        ...
        paths
            .into_iter()
            .map(|p| {
                arrows_to_path_to_arrows(
                    p,
                    times - 1,
                    &mut cache, /*&mut FxHashMap::default()*/
                )
            })
            .fold(vec![], |acc, p| {
...
pub fn process(input: &[u8]) -> usize {
    ...
    (0..5)
        ...
        .map(|i| {
            ...
            for path in paths {
                min = arrows_to_path_to_arrows(path, 10, &mut FxHashMap::default())
                    .len()
                    .min(min);
            }
            println!("Finished {}", i);
